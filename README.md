...
管理頁的每張影音卡提供鉛筆「編輯」與垃圾桶「從網站刪除」按鈕。修改後請按「下載更新後的 media.json」，再替換 GitHub 上的同名檔案；這樣其他瀏覽器才會同步。刪除網站卡片不會刪除 Backblaze 原始檔。

播放器若無法讀取來源，會顯示詳細錯誤與「直接開啟原始影片檢查」按鈕，方便判斷 404、Unauthorized 或不支援的編碼。
